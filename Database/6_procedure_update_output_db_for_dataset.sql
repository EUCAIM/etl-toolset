...
BEGIN
	...
	SELECT itmt.Identifier, ocp.patient_id, CategoryEUCAIM, TumorMarkerEUCAIM, ValueAsNumber, ValueAsConceptEUCAIM, ValueAsConceptUnitEUCAIM, CAST(DateOfMarker AS date)
	...
	SELECT iltr.Identifier, ocp.patient_id, codeEUCAIM, ValueAsConceptEUCAIM, ValueAsNumber, ValueUnitEUCAIM, CAST(DateOfTestResult AS date), OffsetFromDiagnosis, OffsetUnitOriginal
	...
	SELECT ircs.TreatmentIdentifier, ocp.patient_id, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(PerformedDate as date), RadiotherapyEUCAIM
	FROM eucaim_cdm_ingestion.RadiotherapyCourseSummary ircs
	JOIN eucaim_cdm_ingestion.CancerPatient icp ON ircs.PatientIdentifier = icp.Identifier
	JOIN eucaim_cdm_output.patient ocp ON icp.Identifier = ocp.patient_id
	WHERE icp.DatasetIdentifier = p_dataset_id;
	...
	SELECT isp.TreatmentIdentifier, ocp.patient_id, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(PerformedDate as date), ProcedureEUCAIM
	FROM eucaim_cdm_ingestion.SurgicalProcedure isp
	JOIN eucaim_cdm_ingestion.CancerPatient icp ON isp.PatientIdentifier = icp.Identifier
	JOIN eucaim_cdm_output.patient ocp ON icp.Identifier = ocp.patient_id
	WHERE icp.DatasetIdentifier = p_dataset_id;
	...
	SELECT iip.ProcedureIdentifier, opcc.patient_id, opcc.cancer_condition_id, ImagingProcedureEUCAIM, iip.OffsetFromDiagnosis, iip.OffsetUnitEUCAIM, cast(PerformedDate as date), ImagingProcedureCategoryCodeEUCAIM
	...
	INSERT INTO eucaim_cdm_output.procedure(procedure_id, patient_id, cancer_condition_id, procedure_code, procedure_offset_from_diagnosis, procedure_offset_unit, procedure_date, procedure_category)
	SELECT crp.ProcedureIdentifier, opcc.patient_id, opcc.cancer_condition_id, ProcedureEUCAIM, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(PerformedDate as date), ProcedureCategoryCodeEUCAIM
    ...
    SELECT iis.ImageStudyUID, op.procedure_id, op.patient_id, iis.ImagingTimepoint, iis.OffsetFromDiagnosis, iis.OffsetUnitEUCAIM, cast(iis.AcquisitionDate as date), iis.NumberOfSeries, iis.NumberOfInstances
    ...
    SELECT iise.ImageSeriesUID, ois.study_uid, ImageSeriesNumber, Description, Manufacturer, cast(iise.AcquisitionDate as date), modality, iise.BodyPart
	...
	SELECT iep.Identifier, PatientIdentifier, TypeEUCAIM, EpisodeNumber, cast(StartDate as date), cast(EndDate as date)
