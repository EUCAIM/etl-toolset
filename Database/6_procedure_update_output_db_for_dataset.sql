...
BEGIN
	...
	INSERT INTO eucaim_cdm_output.health_status_assessment(patient_id, health_status_assessment_code, health_status_assessment_value_as_number, health_status_assessment_value_as_concept, health_status_assessment_value_unit)
	...
	INSERT INTO eucaim_cdm_output.tumor_marker_test(patient_id, tumor_marker_test_category, tumor_marker_test_code, tumor_marker_test_value_as_number, tumor_marker_test_value_as_concept, tumor_marker_test_value_unit, tumor_marker_test_date)
	...
	INSERT INTO eucaim_cdm_output.family_member_history(patient_id, family_member_history_relationship, family_member_history_condition_code, family_member_history_onset_age, family_member_history_onset_age_unit)
	...
	INSERT INTO eucaim_cdm_output.treatment(patient_id, treatment_type, TreatmentIdentifier)
		...
		JOIN eucaim_cdm_output.patient ocp ON icp.Identifier = ocp.patient_id
		WHERE icp.DatasetIdentifier = p_dataset_id
		returning treatment_id, patient_id, TreatmentIdentifier)
	...
	SELECT inserted.treatment_id, inserted.patient_id, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(PerformedDate as date), RadiotherapyEUCAIM
	FROM eucaim_cdm_ingestion.RadiotherapyCourseSummary ircs
	...
	SELECT inserted.treatment_id, inserted.patient_id, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(PerformedDate as date), ProcedureEUCAIM
	FROM eucaim_cdm_ingestion.SurgicalProcedure isp
	...
	SELECT inserted.treatment_id, inserted.patient_id, OffsetFromDiagnosis, OffsetUnitEUCAIM, cast(DateOfMedication as date), MedicationCodeEUCAIM
	FROM eucaim_cdm_ingestion.CancerRelatedMedication icrm
	JOIN inserted_treatment inserted ON icrm.TreatmentIdentifier = inserted.TreatmentIdentifier;

	-- Update PrimaryCancerCondition
	INSERT INTO eucaim_cdm_output.cancer_condition(Identifier, patient_id, cancer_condition_age_at_diagnosis, cancer_condition_age_unit, cancer_condition_asserted_date, cancer_condition_code, cancer_condition_histology_morphology_behavior, cancer_condition_type)
	...
	FROM (
		SELECT cancer_condition_id, MIN(procedure_id) AS procedure_id
		FROM eucaim_cdm_output.procedure
		WHERE cancer_condition_id IS NOT NULL
		GROUP BY cancer_condition_id
	) AS first_procedure,
	...
	INSERT INTO eucaim_cdm_output.risk_assessment(tumor_id, patient_id, risk_assessment_code, risk_assessment_value_unit, risk_assessment_value_as_concept, risk_assessment_value_as_number)
	...
	JOIN eucaim_cdm_output.tumor ot ON ira.TumorIdentifier = ot.tumor_identifier
	...
	JOIN eucaim_cdm_output.tumor ot ON ito.TumorIdentifier = ot.tumor_identifier
	...
	-- SliceThickness
	INSERT INTO eucaim_cdm_output.image_modality(series_id, series_uid, study_uid, acquisition_parameter_code, acquisition_parameter_value_code, acquisition_parameter_value_number, acquisition_parameter_value_unit)
	...
	-- episode_type_code es un campo codificado del CDM, así que toma TypeEUCAIM y no el
	-- texto original, que es lo que se estaba escribiendo.
	SELECT PatientIdentifier, TypeEUCAIM, EpisodeNumber, cast(StartDate as date), cast(EndDate as date)
