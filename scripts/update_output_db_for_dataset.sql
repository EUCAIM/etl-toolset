...
BEGIN
    ...
    INSERT INTO eucaim_cdm_output.CancerPatient(Identifier, DatasetId, BirthDate, BirthSexCode, Ethnicity, ManagingOrganization, DiagnosticCategoryCode, Deceased, LastContactDate, CauseOfDeath)
	SELECT icp.Identifier, od.Id, CAST(BirthDate AS date), BirthSexEucaim, Ethnicity, ManagingOrganization, DiagnosticCategoryEucaim, Deceased, CAST(LastContactDate AS date), CauseOfDeath
	...
	INSERT INTO eucaim_cdm_output.PrimaryCancerCondition(Identifier, PatientId, AgeOfDiagnosis, AgeUnitCode, AssertedDate, PrimaryCancerConditionCode, HistologyMorphologyBehaviourCode, EpisodeStartDate)
	SELECT ipcc.Identifier, ocp.Id, ipcc.AgeOfDiagnosis, ipcc.AgeUnitEUCAIM, ipcc.AssertedDate, ipcc.PrimaryCancerConditionEUCAIM, ipcc.HistologyMorphologyBehaviourEUCAIM, CAST(ipcc.EpisodeStartDate AS date)
	...
	JOIN eucaim_cdm_output.PrimaryCancerCondition opcc ON ihg.PrimaryCancerConditionIdentifier = opcc.Identifier
	...
	JOIN eucaim_cdm_output.PrimaryCancerCondition opcc ON ics.PrimaryCancerConditionIdentifier = opcc.Identifier
	...
	JOIN eucaim_cdm_output.PrimaryCancerCondition opcc ON icrp.PrimaryCancerConditionIdentifier = opcc.Identifier
	JOIN eucaim_cdm_ingestion.PrimaryCancerCondition ipcc ON icrp.PrimaryCancerConditionIdentifier = ipcc.Identifier
	JOIN eucaim_cdm_ingestion.CancerPatient icp ON ipcc.PatientIdentifier = icp.Identifier
	WHERE icp.DatasetIdentifier = p_dataset_id;
	...
	JOIN eucaim_cdm_output.PrimaryCancerCondition opcc ON iip.PrimaryCancerConditionIdentifier = opcc.Identifier
	...
	JOIN eucaim_cdm_output.PrimaryCancerCondition opcc ON it.PrimaryCancerConditionIdentifier = opcc.Identifier
	...
	JOIN eucaim_cdm_output.ImagingProcedure op ON iis.ImagingProcedureIdentifier = op.ProcedureIdentifier
	...
	JOIN eucaim_cdm_output.ImageStudy ois ON iise.ImageStudyUID = ois.ImageStudyUID
	WHERE iis.DatasetIdentifier = p_dataset_id;


	-- Update flag for this dataset_id	
